      ...
      {page !== 0 && (
        <div className="flex flex-row text-center mb-5 text-white justify-center items-center">
          <button onClick={() => page !== 1 && setPage(page - 1)}>
            {" "}
            <RiArrowLeftSLine className="2xl:w-8 h-8" />
          </button>
          {pageArr.map((pageNum) => (
            <button key={`page-${pageNum}`}>
              <div
                className={`mx-1 ${[
                  pageNum === page
                    ? "font-bold mx-3 text-base 2xl:text-xl opacity-100"
                    : "font-light text-sm 2xl:text-lg opacity-60",
                ]}`}
          ...
          >
            <RiArrowRightSLine className="2xl:w-8 h-8" />
          </button>
        </div>
      )}
    </>
