...
} from "../redux/services/jikanMoeApi";
...
const MangaDetail = () => {
  ...
  const {
    ...
    error: dataError,
  } = useGetMangaByGenreQuery(genreArr.toString());
    ...
    <div className="flex flex-col">
      <div className="flex flex-col md:flex-row">
        <img
          ...
          alt={data?.title}
          className="h-[40vh] md:h-80 2xl:h-[45vh] mr-2 self-center"
        />
        <Detail data={mangaData} />
      </div>
      <section className="font-bold text-white text-[10px] md:text-sm 2xl:text-lg">
        Related Anime
        <Slider isFetching={isFetchingMangaByGenre} data={data?.data} />
      </section>
      <br />
    </div>
  );
